...
# scp fail2ban_script.sh
# Daj uprawnienia plikowi:
# sudo chmod +x fail2ban_script.sh root@ip.ser.we.ra:~/
# Wykonaj plik:
...
if [ $KONF1 == "y" ];then
	...
	sed -i '280 i enabled = true'  /etc/fail2ban/jail.local
        sed -i '394 i enabled = true'  /etc/fail2ban/jail.local

else
	echo -e "Jak to?! Lubujesz się w nieprzekierowanych łączach?"
        sed -i 's/maxretry = 5/maxretry = 3/' /etc/fail2ban/jail.local
	sed -i '280 i enabled = true' /etc/fail2ban/jail.local
fi

echo -e "Skończyliśmy konfigurację, chcesz kończyć instalację? (y/n)"
read END1

if [ $ANSWEAR == "y" ];then
	echo -e "Świetnie! Zrestartujmy więc Fail2ban i działajmy!"
	sudo fail2ban-client reload
else
	echo -e "Nie interesuje mnie to... restart fail2bana i jazda."
	sudo fail2ban-client reload
fi
